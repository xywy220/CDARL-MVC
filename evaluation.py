...
from torchmetrics.clustering import HomogeneityScore, FowlkesMallowsIndex, CompletenessScore, VMeasureScore
...
def clustering_metric(y_true, y_pred, n_clusters, verbose=True, decimals=4):
    ...
    HI = metric(torch.tensor(y_true), torch.tensor(y_pred))

    # FMI
    fmi = FowlkesMallowsIndex()
    FMI = fmi(torch.tensor(y_true), torch.tensor(y_pred))

    # CI
    ci = CompletenessScore()
    CI = ci(torch.tensor(y_true), torch.tensor(y_pred))

    # VM
    vm = VMeasureScore()
    VM = vm(torch.tensor(y_true), torch.tensor(y_pred))


    return dict({'ACC': accuracy, 'NMI': nmi, 'ARI': ari, 'HI': HI, 'FMI': FMI, 'CI': CI, 'VM': VM}), confusion_matrix
